...
import type { GameState, LogEntry, LogCategory } from '../types';
...
export function xpForLevel(n: number): number {
  return Math.floor(80 * n * Math.pow(1.09, n));
}

export function levelFromXp(xp: number): [number, number, number] {
...
export function getSkillLevel(state: GameState, skill: string): [number, number, number] {
...
export function seasonTimeRemaining(state: GameState): number {
  const elapsed = Date.now() / 1000 - state.seasonStartedAt;
