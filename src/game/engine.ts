...
import type { GameState, ActiveAction } from '../types';
import { SKILL_LABELS, levelFromXp, addLog, tickSeasons, getSeason, SEASON_BONUSES, WORLD_MILESTONES, LEGENDARY_SETS, SEASONS } from './state';
import { findAction } from './skills';
import { rollDrops, TRACKABLE_DROPS, RARE_TIERS, openLootbox } from './drops';
import type { DropTier } from './drops';

// ── Buff effect definitions ────────────────────────────────────────────────
// Structured so the engine can read them without parsing display strings.
...
const BUFF_EFFECTS: Record<string, BuffEffect> = {
  // Shop boosts
...
export function actionProgress(state: GameState): number {
  ...
  return Math.min((Date.now() / 1000 - a.startedAt) / a.duration, 1);
}
